...
void Player::add_peace(int scoket, Board &board, Dice &dice, std::vector<PreviousMove*>& moves) {
    char message[128];
    recv(scoket, message, 128, 0);               //string format to get ("x y")
    int x, y, flag;
    std::string stmessage(message);
    for (int i = 0; i < stmessage.size(); i++) {
        if (stmessage[i] == ' ') {
    ...
    board.insert_peace(this, x, y, dice, moves);
}